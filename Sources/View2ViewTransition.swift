...
public protocol View2ViewTransitionPresenting {
    ...
    func initialFrame(userInfo: [String: Any]?, isPresenting: Bool) -> CGRect
    
    /// Return initial transition view.
    /// - parameter userInfo: user info
    /// - parameter isPresenting: transition is present or not
    func initialView(userInfo: [String: Any]?, isPresenting: Bool) -> UIView
    
    /// Prepare initial transition view (optional).
    /// - parameter userInfo: user info
    /// - parameter isPresenting: transition is present or not
    func prepareInitialView(userInfo: [String: Any]?, isPresenting: Bool) -> Void
}
...
public protocol View2ViewTransitionPresented {
    ...
    /// - parameter userInfo: user info
    /// - parameter isPresenting: transition is present or not
    func prepareDestinationView(userInfo: [String: Any]?, isPresenting: Bool) -> Void
}
